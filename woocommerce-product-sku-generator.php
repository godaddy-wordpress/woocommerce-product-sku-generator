...
if ( ! WC_SKU_Generator::is_plugin_active( 'woocommerce.php' ) ) {
	return;
}
...
class WC_SKU_Generator {
	...
	/**
	 * Initializes the plugin, bailing if any required conditions are not met,
	...
	public function initialize() {
		...
		if ( ! $this->is_minimum_wc_version_met() ) {
			// halt functionality
			return;
		}
		...
		if ( is_admin() ) {
			...
			add_action( 'woocommerce_product_write_panel_tabs', [ $this, 'maybe_disable_variation_skus' ], 10, 2 );

			// generate SKUs when variations are saved via ajax
			add_action( 'woocommerce_ajax_save_product_variations', [ $this, 'ajax_maybe_generate_variation_skus' ] );
		}

		if ( is_admin() && ! is_ajax() ) {

			// add settings
			add_filter( 'woocommerce_products_general_settings', [ $this, 'add_settings' ] );
			...
			add_action( 'woocommerce_product_bulk_edit_save', [ $this, 'maybe_save_sku' ] );
			add_action( 'woocommerce_process_product_meta',   [ $this, 'maybe_save_sku' ], 100, 2 );

			add_action( 'woocommerce_save_product_variation', [ $this, 'maybe_save_varation_sku' ] );

			// disable SKU fields when being generated by the plugin
			add_action( 'init', [ $this, 'maybe_disable_skus' ] );
	...
	/**
	 * Returns the currently installed version of WooCommerce.
	 *
	 * @since 2.4.6-dev.1
	 * @return string/null The WooCommerce version number or null.
	 */
	...
	/**
	 * Determines if the minimum WooCommerce version is met.
	 *
	 * @since 2.4.6-dev.1
	 * @return boolean true if the required version is met, false otherwise
	 */
	private function is_minimum_wc_version_met() {

		$version_met = true;

		// if a plugin defines a minimum WC version, render a notice and skip loading the plugin
		if ( defined( 'self::MIN_WOOCOMMERCE_VERSION' ) && version_compare( self::get_wc_version(), self::MIN_WOOCOMMERCE_VERSION, '<' ) ) {
